// ═══════════════════════════════════════════════════════════════════
//  NPChain Mainnet Node v1.0.0
//
//  Production mainnet node. Mines blocks using Proof-of-NP-Witness
//  consensus with real SHA3-256 hashing, CDCL+WalkSAT solving,
//  and O(n) verification. Post-quantum cryptographic stack.
//
//  Compile:
//    g++ -std=c++20 -O2 -pthread -I include -I src \
//        src/mainnet_node.cpp \
//        src/crypto/hash.cpp \
//        src/crypto/dilithium.cpp \
//        -o npchain_mainnet -lws2_32
//
//  Run:
//    ./npchain_mainnet --address cert1YOUR_ADDRESS
//    ./npchain_mainnet --address cert1... --seed SEED_IP:9333
// ═══════════════════════════════════════════════════════════════════
...
struct ChainState {
    ...
    uint32_t current_difficulty() const {
        ...
        // Adjust every 144 blocks (mainnet)
        constexpr uint64_t WINDOW = 144;
        constexpr uint64_t TARGET_TIME = 60;  // 60s mainnet blocks
    ...
    uint64_t block_reward() const {
        // Mainnet: 100B Certs/year at 60s blocks = ~190,258 Certs/block
        constexpr uint64_t MAINNET_BLOCKS_PER_YEAR = 525'600;
        constexpr uint64_t BASE_UNITS = 1'000'000ULL;  // 6 decimals
        constexpr uint64_t ANNUAL_CAP = 100'000'000'000ULL * BASE_UNITS;
        return ANNUAL_CAP / MAINNET_BLOCKS_PER_YEAR;
    }
...
void handle_rpc_client(SOCKET client) {
    ...
    if (path == "/api/v1/status" || path == "/") {
        ...
        ss << "{\"chain_id\":\"CRT\",\"network\":\"mainnet\""
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--help") {
            std::cout << "Usage: npchain_mainnet [options]\n\n"
                      ...
                      << "  --p2p-port <port>      P2P listen port (default: 9333)\n"
                      << "  --seed <host:port>     Seed node to connect to (can use multiple)\n"
                      << "  --block-time <secs>    Target block time (default: 60)\n"
                      << "  --fast                 No delay between blocks (testing only)\n"
                      << "\nExamples:\n"
                      << "  # Solo mining:\n"
                      << "  ./npchain_mainnet --address cert1abc...\n\n"
                      << "  # First node (seed):\n"
                      << "  ./npchain_mainnet --address cert1abc... --p2p-port 9333\n\n"
                      << "  # Second node (connects to first):\n"
                      << "  ./npchain_mainnet --address cert1def... --p2p-port 9334 --seed localhost:9333\n";
            return 0;
        }
    }
